...
function run_silicon_pbe0(T ;Ecut=5, grid_size=15, spin_polarization=:none, kwargs...)
    ...
    Si = ElementPsp(silicon.atnum, psp=load_psp(silicon.atnum, functional="pbe0", family="hgh"))
    model = model_Hybrid(Array{T}(silicon.lattice), [Si => silicon.positions], [:pbe0],
                      spin_polarization=spin_polarization)
    basis = PlaneWaveBasis(model, Ecut, silicon.kcoords, silicon.ksymops; fft_size=fft_size)

    spin_polarization == :collinear && (ref_pbe = vcat(ref_pbe0, ref_pbe0))
    run_scf_and_compare(T, basis, ref_pbe0, ref_etot;
